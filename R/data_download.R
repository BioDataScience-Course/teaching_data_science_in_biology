...
# Indicate the root folder where these data must be downloadare located here:

root <- svMisc::pcloud() # Replace this if you have a different directory!
#root <- "~/Desktop/zenodo_repos/"

if (!fs::dir_exists(root))
  stop("The directory 'root' (", root, ") do not exist. ",
    "You must indicate an existing directory!")
#fs::dir_create(root) # if you want create the folders
...
resources <- paste0(c(
  "assessments", "challenge", "courses", "git_log", "h5p",
  "learnr", "lessons", "projects", "shiny", "support",
  "users", "wooclap"),".csv")
all_resources <- c(resources, "datapackage.json", "README.md")

for(i in seq_along(all_resources)) {
  if(!fs::file_exists(path(data_dir, all_resources[i]))){
    download.file(
      url = paste0(data_url, all_resources[i]),
...
resources <- paste0(c(
  "assessment", "courses", "exam", "git_log",
  "lessons", "projects", "support", "users"),".csv")
all_resources <- c(resources, "datapackage.json", "README.md")
