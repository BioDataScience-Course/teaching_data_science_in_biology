...
```{r}
time_interval <- interval(as.time("2020/02/01"), as.time("2021/01/15"))

bind_rows(log19, log) %>.%
  distinct(., commit, .keep_all = TRUE) %>.%
  filter(., is.na(from)) %>.%
  filter(., date %within% time_interval) %>.%
  mutate(., week = acad_w(date, label = TRUE), period = acad_2w(date, label = TRUE)) -> git_red

bind_rows(support19, support) %>.%
  filter(., date %within% time_interval) %>.%
  mutate(., week = acad_w(date, label = TRUE), period = acad_2w(date, label = TRUE)) %>.%
  mutate(., sender = c("student", "teacher")[
    (user %in% c("@display", "@teacher", "id2", "id3", "id6", "id78") |
    (user == "id121" & course != "S-BIOG-025")) + 1]) %>.%
  drop_na(., sender) %>.%
  mutate(., sender = as.factor(sender)) -> sup

git_red$period <- as.factor(git_red$period)
sup$period <- factor(sup$period, levels = levels(git_red$period))

git_red %>.%
  group_by(., period, user) %>.%
  summarise(., count = n()) %>.%
  chart(., count ~ period) +
  geom_boxplot() +
  labs(y = "Number of \n commits", x = "Period") +
  theme(axis.text.x = element_text(angle = 90)) +
  scale_x_discrete("period", drop=FALSE) -> pcommit


sup %>.%
  filter(., course != "S-BIOG-025") %>.%
  filter(., sender != "teacher") %>.%
  group_by(., period, user) %>.%
  summarise(., count = n()) %>.%
  chart(., count ~ period ) +
  geom_boxplot() +
  labs(y = "Number of \n messages", x = "Period") +
  theme(axis.text.x = element_text(angle = 90)) +
  scale_x_discrete("period", drop=FALSE) ->psup

combine_charts(list(pcommit, psup), nrow = 2, common.legend = TRUE)
```

The academic year is divided into work periods of approximately 2 weeks. The courses of the second term of the year 2019-2020 start in period Y1P09 and the courses of the first term of the year 2020-2020 start in period Y2P01. The first lockdown started in period Y1P11 and the second in Y2P03. During the first lockdown, support rapidly switched to the proposed channels, by email and via Discord.

In 2019-2020, a drop in the number of commits is observed in Y1P11 which corresponds to the beginning of the first lockdown. After one period of transition, students start again to work and interact with teachers. The last hours of lessons are given in period Y1P13. The higher number of commits in Y1P16 and Y1P17 corresponds to the third term.

In 2020-2021, the number of commits gradually increases from Y2P01 to Y2P03 and stabilises until Y2P08. The number of messages increases from the beginning of the second containment (Y2P03). There is no lag period. The number of commits remains constant. The students no longer needed an adaptation period. 
