...
users19 <- read(pcloud_crypto("sdd_users/users_2019-2020_Q1.csv")) 
users <- read(pcloud_crypto("sdd_users/users_2020-2021_Q1.csv")) 
## pseudo date
...
Nous utilisons une machine virtuelle préconfiguré avec R et RStudio afin de permettre aux étudiants de fixer les concepts théoriques directement au travers d'exercices pratique.  

L'utilisation d'un software professionnel est indispensable pour analyser des données écologique....
...
left_join(us_tab, exercice_tab, by = c("course"))
```
...
q1_regular <- filter(q1, user %in% users19$user[users19$state =="regular"])
